...
try {
	...
	$cliente = array('name' => $nombre_cliente, 'phone_number' => $telefono, 'email' => $correo);
	...
	switch($tipo) {
		...
		case 'promo' :
			$tipoN = 6;
			$descripcion = "Promo x 1 mes locales:";
			$precio = 500;
			$fecha1 = date('Y-m-d');
			$dif = compararFechas($fecha1, $fechaInicial);
			if ($dif < 0) {
	...
	foreach ($json as $key => $value) {
		...
		$campos = "id_cliente,id_local,tipo_pago,fecha_pago,fecha_inicio,id_orden,estado_pago,caducidad";
		$datos = "$id_cliente,$id_local,$tipoN,'$fecha_pago','$fechaInicial',$id_orden,0,'$caducidadBd'";
  ...
  ");
	...
	$mail -> send();
	unset($mail);
	$mensaje = "Tianguismex te invita a realizar el pago haz clic en el link " . $link;
